...
app = FastAPI(
    lifespan=lifespan,
    title="CanBuildAI Surrogate Model API",
    description="API for Building Energy Surrogate Model predictions",
    version="3.0.0",
...
# Register surrogate model routes
...
async def health_check():
    return JSONResponse(status_code=200, content={
        ...
        "message": "Surrogate Model API is healthy"
    })

@app.get("/")
async def root():
    return PlainTextResponse("CanBuildAI Surrogate Model API v3.0 - Use /docs for API documentation")

if __name__ == "__main__":
    import uvicorn
    import os
    port = int(os.getenv("PORT", 8000))
    uvicorn.run("api.main_surrogate:app", host="0.0.0.0", port=port)
